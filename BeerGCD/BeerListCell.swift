...
class BeerListCell: UITableViewCell {
    ...
    func configure(with beer: Beer){
        let imageURL = URL(string: beer.imageURL ??  "") // 만약에 빈 값으면 빈 값으로
        beerImageView.kf.setImage(with: imageURL, placeholder: #imageLiteral(resourceName: "kingfisher-10.jpg")) // placehoder 빈 값일때 넣을 기본 이미지
        nameLabel.text = beer.name ?? "이름 없는 맥주"
        taglineLabel.text = beer.tagLine
        
        accessoryType = .disclosureIndicator // 옆에 버튼
        selectionStyle = .none
    }
}
